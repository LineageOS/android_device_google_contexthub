...
#define AP_INT             NANOHUB_INT_WAKEUP
...
// Either determine the sensor type from the EVT number
// Or determine the sensor type from EVT data.
static int sensorToIndex(uint32_t evtType)
{
    switch (evtType) {
        case EVT_SENSOR_ACC_CONFIG:
        case EVT_SENSOR_ACC_ACTIVATE:
            return ACC_IDX;
        case EVT_SENSOR_GYR_CONFIG:
        case EVT_SENSOR_GYR_ACTIVATE:
            return GYR_IDX;
        case EVT_SENSOR_MAG_CONFIG:
        case EVT_SENSOR_MAG_ACTIVATE:
            return MAG_IDX;
        default:
...
static bool bmi160Isr1(struct ChainedIsr *isr)
{
    if (mIntTime == 0) {
        ...
        osLog(LOG_INFO, "        Delta Time between last Int is: %lu\n",
                (long unsigned int)(temp_Int_time - mIntTime));
...
static bool accFirmwareUpload()
{
    sensorSignalInternalEvt(mTask.sensors[ACC_IDX].handle,
            SENSOR_INTERNAL_EVT_FW_STATE_CHG,
            mTask.sensors[ACC_IDX].rate, mTask.sensors[ACC_IDX].latency[0]);
    return true;
}
...
{
    osLog(LOG_INFO, "In accPower()\n");
    if (on) {
        osLog(LOG_INFO, "In accPower() ON path\n");
...
static bool magPower(bool on)
{
    ...
    osLog(LOG_INFO, "Calling power change for MAG\n");
...
static bool accSetRate(uint32_t rate, uint64_t latency)
{
    ...
    osLog(LOG_INFO, "Setting ACC rate to %u\n", (unsigned int)rate);
    ...
    int odr = computeOdr(actual_rate);
    if (odr == -1)
        return false;
    ...
    osLog(LOG_INFO, " THE RATE REG is set to: %02x\n", odr);
    spiBatchTxRx(&mTask.mode, sensorSpiCallback, &mTask.sensors[ACC_IDX]);
    return true;
}
...
static bool magSetRate(uint32_t rate, uint64_t latency)
{
    ...
    int odr = computeOdr(rate);
    if (odr == -1)
        return false;

    // set the rate for MAG
    SPI_WRITE(BMI160_REG_MAG_CONF, odr);
    osLog(LOG_INFO, "Calling rate change for MAG\n");
    spiBatchTxRx(&mTask.mode, sensorSpiCallback, &mTask.sensors[MAG_IDX]);
    return true;
}
...
{
    osLog(LOG_INFO, "In configEvent()\n");

    if (mTask.state < SENSOR_IDLE) {
        osLog(LOG_ERROR, "Chip not initialized yet\n");
    }else if (mTask.state == SENSOR_IDLE) {
        if (ConfigData->enable == 0 && mSensor->active[0]) {
            // if we are on, cmd is to turn off, release sensor
            sensorRelease(ConfigData->clientId, mSensor->handle);

        } else if (ConfigData->enable == 1 && !mSensor->active[0]) {
            osLog(LOG_INFO, "Calling sensorRequest()\n");
            // if we are off, cmd is to turn on, request sensor
            sensorRequest(ConfigData->clientId, mSensor->handle, ConfigData->rate, ConfigData->latency);
    ...
    while (size > 0) {
        ...
        if (fh_mode == 1) {
            ...
            if (fh_param == 1) {
                // sensortime frame
                if (size >= 3) {
                    //osLog(LOG_INFO, "GOT TTTIIIIMMMMMEEE fram!\n");
...
static void int1Handling(void)
{
    switch (mTask.fifo_state) {
        ...
        case FIFO_READ_DATA:
            ...
            osLog(LOG_INFO, "fifo size: %d, ERR: %02x, PMU: %02x\n",
                    mTask.xferCnt, mTask.rxBuffer[4], mTask.rxBuffer[6]);
    ...
    switch (mTask.calibration_state) {
        ...
        case CALIBRATION_SET_OFFSET:
            ...
            osLog(LOG_INFO, "ACCELERATION OFFSET is %02x  %02x  %02x\n",
                    (unsigned int)mTask.sensors[ACC_IDX].offset[0],
                    (unsigned int)mTask.sensors[ACC_IDX].offset[1],
                    (unsigned int)mTask.sensors[ACC_IDX].offset[2]);
    ...
    switch (mTask.state) {
        ...
        case SENSOR_INITIALIZING:
            ...
            if (mTask.init_state == INIT_DONE) {
                ...
                //osEnqueueEvt(EVT_SENSOR_ACC_CALIBRATE, NULL, NULL, false);
                osLog(LOG_INFO, "Done initialzing, system IDLE\n");
                mTask.state = SENSOR_IDLE;
                //activateCmd.active[0] = true;
                //osEnqueueEvt(EVT_SENSOR_ACC_ACTIVATE, &activateCmd, NULL, false);
                //osEnqueueEvt(EVT_SENSOR_GYR_ACTIVATE, &activateCmd, NULL, false);
                //osEnqueueEvt(EVT_SENSOR_MAG_ACTIVATE, &activateCmd, NULL, false);
                //timTimerSet(100000000, 100, 100, enableAllSensors, NULL, true);
            } else {
                sensorInit();
            }
...
/*
                int i;
                osLog(LOG_INFO, "Done fifo transfer. Print some bytes:\n");
                for (i = 250; i <= 270; i++) {
                    osLog(LOG_INFO, "%02x\n", mTask.rxBuffer[i]);
                }
*/
...
static void handleEvent(uint32_t evtType, const void* evtData)
{
    uint64_t currTime;
    struct BMI160Sensor *mSensor;
    struct ConfigStat *ConfigData;

    switch (evtType) {
        case EVT_SENSOR_ACC_CONFIG:
        case EVT_SENSOR_GYR_CONFIG:
        case EVT_SENSOR_MAG_CONFIG:
            mSensor = &mTask.sensors[sensorToIndex(evtType)];
            ConfigData = (struct ConfigStat *)evtData;
            configEvent(mSensor, ConfigData);
            break;
        case EVT_APP_START:
            mTask.state = SENSOR_BOOT;
            osEventUnsubscribe(mTask.tid, EVT_APP_START);
            osEventSubscribe(mTask.tid, EVT_SENSOR_ACC_CONFIG);
            osEventSubscribe(mTask.tid, EVT_SENSOR_GYR_CONFIG);
            osEventSubscribe(mTask.tid, EVT_SENSOR_MAG_CONFIG);
            osEventSubscribe(mTask.tid, EVT_SENSOR_ACC_CALIBRATE);
            osEventSubscribe(mTask.tid, EVT_SENSOR_GYR_CALIBRATE);
